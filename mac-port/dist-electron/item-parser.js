export class ItemParser {
    static parse(text) {
        ...
        for (const line of lines) {
            ...
            if (item.requirements) {
                if (line.includes('Level:'))
                    item.requirements.level = parseInt(line.match(/Level: (\d+)/)?.[1] || '0');
                if (line.includes('Str:'))
                    item.requirements.str = parseInt(line.match(/Str: (\d+)/)?.[1] || '0');
                if (line.includes('Dex:'))
                    item.requirements.dex = parseInt(line.match(/Dex: (\d+)/)?.[1] || '0');
                if (line.includes('Int:'))
                    item.requirements.int = parseInt(line.match(/Int: (\d+)/)?.[1] || '0');
            }
            // Physical Damage
        ...
        const isMod = (line) => {
            if (line === item.name || line === item.baseType || line === item.rarity || line === item.class)
                return false;
            for (const p of nonModPrefixes)
