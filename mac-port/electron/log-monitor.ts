...
export class LogMonitor extends EventEmitter {
  ...
  async jumpToNext(eventType: 'area-changed' | 'slain' | 'whisper' | 'level-up' | 'login') {
    ...
    for (let i = this.simulationIndex + 1; i < this.simulationLines.length; i++) {
      ...
      let found = false;
      if (eventType === 'area-changed' && (line.includes('Generating level') || line.includes('You have entered'))) found = true;
      if (eventType === 'slain' && (line.includes('has been slain') || line.includes('You have been slain') || line.includes('has committed suicide'))) found = true;
      if (eventType === 'level-up' && line.includes('is now level')) found = true;
      if (eventType === 'whisper' && line.includes('@From')) found = true;
      
      // Special Login check
      if (eventType === 'login' && (
          (line.includes(' connected to ') && line.includes('.login.')) || 
          line.includes('*****') || 
  ...
  async calculateStats() {
    ...
    let totalDeaths = 0;
    const levelHistory: { level: number; time: Date; duration?: string }[] = [];
    const areaCounts: Record<string, number> = {};
    let lastLevelTime: Date | null = null;
  ...
  private readNewContent(start: number, end: number) {
