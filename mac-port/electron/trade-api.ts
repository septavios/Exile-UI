...
export class TradeAPI {
  private static rateLimits: { [key: string]: number } = {};
  ...
  static async search(item: any, league: string) {
    if (!item) return null;
    ...
    if (item.rarity === 'Unique' && item.name) {
       ...
       // Pseudo DPS
       if (item.dps) {
          if (item.dps.phys > 0) stats.push({ id: 'pseudo.pseudo_physical_dps', value: { min: Math.floor(item.dps.phys * 0.9) } });
          if (item.dps.elem > 0) stats.push({ id: 'pseudo.pseudo_elemental_dps', value: { min: Math.floor(item.dps.elem * 0.9) } });
       }
  ...
  static async fetchResults(id: string, itemIds: string[]) {
